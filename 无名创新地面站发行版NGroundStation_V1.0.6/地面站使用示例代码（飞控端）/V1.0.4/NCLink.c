...
void NCLink_SEND_StateMachine(void)
{
	...
	if(!NCLink_Send_Check_Status_Parameter())//判断地面站有无请求数据、是否需要发送应答
	{
		...
		{
			NCLink_Cnt=9;
			NCLink_Send_Userdata(Roll_Observation,Pitch_Observation,Yaw_Observation,Roll,Pitch,Yaw);	
		...
		else if(NCLink_Cnt==9)//传感器校准原始数据
		{
		  ...
		  NCLink_Send_CalRawdata1(Gyro_Safety_Calibration_Flag,
			                       WP_Sensor.gyro_raw.x*0.030488f,
														 WP_Sensor.gyro_raw.y*0.030488f,
														 WP_Sensor.gyro_raw.z*0.030488f,
